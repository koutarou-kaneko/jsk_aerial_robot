  ...
  if(lqi_mode_)
    {
      ...
      //ROS_INFO("spinal reconstructed yaw PI term: [%f, %f, %f, %f]", yaw_pi_term_[0], yaw_pi_term_[1], yaw_pi_term_[2], yaw_pi_term_[3];
    }
  else
    { /* dynamics inversion */
      switch (uav_model_)
              ...
              if(max_yaw_term_index_ != -1 && base_throttle_term_[0] > 0 )
                {
                  ...
                  if(residual_term < 0)
                    {
                      yaw_decreasing_rate =  residual_term / (yaw_term_[thrust_index] / rotor_devider_);
                      //ROS_WARN_STREAM("saturation lev.1 bacause of " << (type==0?(std::string("min")):(std::string("max"))) << " thrust violation, restrict yaw control");
                    }
                }
              else
                {

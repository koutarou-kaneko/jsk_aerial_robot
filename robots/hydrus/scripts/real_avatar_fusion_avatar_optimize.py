...
class fusion_avatar_optimize():
  ...
  def __init__(self):
    ...
    self.max_fc_rp_min_sub = rospy.Subscriber('/hydrus/opt_max_fcrpmin', Float64, self.max_fc_rp_min_Cb)
    self.fc_rp_min_sub = rospy.Subscriber('/hydrus/fc_rp_min', Float64, self.fc_rp_min_Cb)
    ...
    self.fusion_torque_balance_pub = rospy.Publisher('/hydrus/fusion_torque_balance', Float64, queue_size=10)
    self.fusion_pos_balance_pub = rospy.Publisher('/hydrus/fusion_pos_balance', Float64, queue_size=10)
    ...
    while not rospy.is_shutdown():  
      if self.avatar_joint_angles.position != []:
        if self.Hovering==True or self.debug==True:
          ...
          self.pos_balance = 0.8
          self.torque_balance = 0.6

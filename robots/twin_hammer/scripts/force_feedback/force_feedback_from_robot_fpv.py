...
class force_feedback_from_robot():
  ...
  def robot_mocap_cb(self,msg):
    ...
    rot = R.from_quat(q)
    self.robot_att = rot.as_euler('xyz', degrees=False)
    R_mat = rot.as_matrix()
    self.Ad_R_robot = np.block([
    [R_mat, np.zeros((3, 3))],
    [np.zeros((3, 3)), R_mat]
  ...
  def device_mocap_cb(self,msg):
    ...
    q = [msg.pose.orientation.x,msg.pose.orientation.y, msg.pose.orientation.z, msg.pose.orientation.w]
    rot = R.from_quat(q)
    self.device_att = rot.as_euler('xyz', degrees=False)
    if self.device_init_yaw == None:
      self.device_init_yaw = self.device_att[2]
    R_mat = rot.as_matrix()
    self.Ad_R_inv_device = np.block([
  ...
  def main(self):
    ...
    while not rospy.is_shutdown():
      ...
      if self.device_init_yaw != None:
        wrench_device_local_x, wrench_device_local_y = rotate_xy_world_to_body(
          haptics_wrench[0], haptics_wrench[1], self.device_init_yaw)
        haptics_wrench[0] = wrench_device_local_x
        haptics_wrench[1] = wrench_device_local_y   
